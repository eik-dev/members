...
import { ArrowUpTrayIcon, PaperAirplaneIcon, ArrowDownOnSquareIcon, CogIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
...
export default function Page() {
    ...
    let generate = (e)=>{
        e.preventDefault();
        postData((response)=>{setData(response.data)},{
            data:filtered,
            training:1
        },'/training/members')
    }
    let download = (e)=>{
        ...
        filtered.forEach(participant => {
            getFile('/training/download',{number:participant.number})
        });
    }
    let email = (e)=>{
        e.preventDefault();
    ...
    <div className=''>
        ...
        <div className='hidden md:flex justify-center bg-gray-200/70 py-8'>
            {/* <TrainingCert data={{}} /> */}
            ...
            <div className='w-1/2 max-w-1/2 max-h-96 overflow-y-scroll'>
                <table className="w-full text-sm lg:text-xs 2xl:text-sm text-left table-auto">
                    <thead className="capitalize bg-gray-100 sticky top-0">
                        <tr>
                        {
                            ['','','name','email','member number'].map((th, index) => {
